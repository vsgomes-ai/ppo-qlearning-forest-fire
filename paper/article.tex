...
política aleatória (\SI{52,7}{\percent}).
Testes de Student ($t$~pareados) nas mesmas sementes confirmam diferenças
significativas do PPO em biomassa frente às três alternativas ($p<0{,}05$).
O resultado indica que, quando o fogo
...
(\SI{54.7}{\percent}), and the random policy (\SI{52.7}{\percent}).
Paired $t$-tests on shared seeds confirm significant PPO gains in biomass
against all three alternatives ($p<0.05$).
The result
...
Além de constituir um problema de controle, este estudo mostra como autômatos
celulares oferecem um ambiente experimental particularmente propício para
investigar algoritmos de decisão em sistemas espaciais complexos: as regras de
atualização são totalmente conhecidas e auditáveis, o que permite isolar com
precisão o efeito da representação de estado sobre o desempenho, algo mais
difícil de garantir em ambientes de RL com dinâmica opaca.
...
Como as sementes de teste são compartilhadas, as diferenças por episódio formam
pares naturais.
Aplicamos testes~$t$~de Student bilaterais pareados à fração de árvores
preservadas, comparando o PPO a cada alternativa (Tabela~\ref{tab:paired}).
Todos os três contrastes são significativos a $p<0{,}05$:
versus aleatório, $\Delta=+0{,}083$, $t(99)=3{,}71$, $p=3{,}4\times 10^{-4}$;
versus heurística, $\Delta=+0{,}051$, $t(99)=2{,}59$, $p=0{,}011$;
versus \emph{Q-learning}, $\Delta=+0{,}063$, $t(99)=2{,}88$, $p=0{,}0049$.
Os intervalos de confiança de \SI{95}{\percent} para a diferença média
excluem zero em todos os casos.
Os tamanhos de efeito pareados (Cohen~$d_z$) são modestos
($0{,}26$ a $0{,}37$ em biomassa), coerentes com a alta variância entre mapas,
mas o sinal é sistemático sob o protocolo de sementes idênticas.
No retorno acumulado, as diferenças pareadas do PPO são ainda mais nítidas
($p<10^{-4}$ frente a cada baseline).
A Figura~\ref{fig:paired} mostra a distribuição das diferenças
$\Delta=$~PPO $-$ baseline por semente: a massa de pontos situa-se acima de
zero em todos os contrastes, com espalhamento visível, o que ilustra o efeito
sistemático porém modesto reportado na Tabela~\ref{tab:paired}.

\begin{table}[H]
\centering
\caption{Testes~$t$~de Student pareados (bilaterais) na fração de árvores
preservadas: PPO versus cada política, $n=100$ episódios com sementes
$1000$--$1099$.
$\Delta$ é a diferença média (PPO $-$ baseline); $d_z$ é o tamanho de efeito
pareado de Cohen.}
\label{tab:paired}
\begin{tabular}{@{}lcccc@{}}
\toprule
Contraste & $\Delta$ & $t(99)$ & $p$ & $d_z$ \\
\midrule
PPO vs.\ aleatório   & $+0{,}083$ & $3{,}71$ & $3{,}4\times 10^{-4}$ & $0{,}37$ \\
PPO vs.\ heurística  & $+0{,}051$ & $2{,}59$ & $0{,}011$ & $0{,}26$ \\
PPO vs.\ Q-learning  & $+0{,}063$ & $2{,}88$ & $0{,}0049$ & $0{,}29$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.88\textwidth]{figures/fig_paired_diff.pdf}
\caption{Diferenças pareadas de biomassa preservada por semente
($\Delta=$~PPO $-$ baseline) nos três contrastes.
Cada ponto é um dos $100$ episódios de teste com semente compartilhada.
A linha horizontal em zero marca equivalência; valores positivos favorecem o PPO.
O losango preto indica a diferença média em cada contraste.}
\label{fig:paired}
\end{figure}
...
Os testes pareados da Tabela~\ref{tab:paired} quantificam esse deslocamento
como diferença média estatisticamente detectável, e não apenas como um ou dois
episódios outliers.
...
Enfatizamos que esse episódio é um caso ilustrativo de melhor cenário
(\emph{best case}), escolhido porque o contraste espacial é visualmente claro;
ele \emph{não} é representativo da média da Tabela~\ref{tab:results}
(\SI{61}{\percent} versus cerca de \SI{55}{\percent}--\SI{56}{\percent}).
A figura serve para mostrar um regime em que antecipar o sotavento muda o
desfecho qualitativo do incêndio, enquanto a inferência quantitativa
permanece ancorada nas $100$ sementes compartilhadas e nos testes pareados.
...
\begin{subfigure}[t]{0.32\textwidth}
  ...
  \caption{PPO}
\end{subfigure}
\caption{Renderização 3D de um episódio ilustrativo de melhor cenário
(semente $340$), \emph{não} representativo da média da
...
Esse padrão ilustra um princípio clássico de RL: o desempenho depende tanto
da representação do estado quanto do algoritmo de
aprendizado~\cite{sutton2018}.
Convém, contudo, precisar o que a comparação isola e o que ela mistura.
O \emph{Q-learning} opera em tabela sobre $625$ estados discretos; o PPO
aproxima política e valor com uma rede neural MLP sobre um vetor contínuo de
$77$ dimensões.
Assim, o contraste não é apenas ``observação compacta versus observação
estruturada'': também é ``tabela versus aproximação de função''.
Dar as mesmas $77$ dimensões ao \emph{Q-learning} tabular exigiria
discretização agressiva; um caminho mais limpo para separar observação de
capacidade de modelo seria treinar, no mesmo vetor de $77$ dimensões, um
algoritmo Deep Q-Network (DQN), que utiliza aproximação de função, e comparar
PPO versus DQN.
Não realizamos esse experimento aqui; reconhecemos a confusão parcial nas
limitações e interpretamos o ganho do PPO como evidência conjunta de
representação espacial mais informativa \emph{e} de um otimizador capaz de
explorá-la.
A comparação justa que adotamos (mesmo ambiente, mesmas sementes, mesmo
espaço de ações) ainda isola o eixo informação/capacidade de decisão frente
à física compartilhada do AC.

\subsection{Limitações}
\label{sec:limitacoes}

A grade é moderada ($30\times 30$); o vento é constante, com reforço fixo
$\mathrm{wind\_boost}=2{,}0$; não há regeneração de vegetação nem fogo
subterrâneo.
Não exploramos sensibilidade a intensidades de vento distintas, o que seria
um teste natural de robustez.
A variância entre episódios permanece alta porque a geometria dos três focos
iniciais muda o grau de dificuldade (desvios padrão de biomassa próximos de
$0{,}22$).
Com $n=100$ e diferenças médias de $0{,}05$ a $0{,}08$, o poder estatístico é
limitado: os testes pareados detectam o sinal do PPO, mas os tamanhos de
efeito em biomassa são modestos, e intervalos mais estreitos exigiriam mais
episódios de teste.
A vantagem média do PPO em biomassa é, portanto, real sob o protocolo
pareado, porém numericamente modesta em pontos percentuais; o ganho em
retorno é mais pronunciado.
Além disso, como discutido acima, a comparação \emph{Q-learning} versus PPO
confunde parcialmente tipo de observação e capacidade de aproximação de
função; um método baseado em aproximação de função, como o DQN, na observação
de $77$ dimensões isolaria melhor esses fatores.
Essas ressalvas não anulam o resultado central: sob física com brasas, uma
política com observação espacial estruturada e aproximação neural eleva o
teto de desempenho frente a baselines locais e a um tabular compacto, no
mesmo ambiente.
...
sugerem que, no cenário estudado, em problemas de controle espacial com
propagação não local, investir em representações de estado mais informativas
pode ser tão ou mais importante do que substituir o algoritmo de aprendizado.
...
memória.
Outras extensões diretas são: (i)~variação sistemática de
$\mathrm{wind\_boost}$ e de $p_{\mathrm{spot}}$; (ii)~avaliação com mais
episódios de teste para estreitar intervalos; (iii)~um algoritmo DQN na mesma
observação de $77$ dimensões, para separar melhor representação de
...
Como o protocolo é pareado por semente, também aplicamos testes~$t$~de Student
bilaterais pareados à biomassa e ao retorno, tomando o PPO como referência
frente a cada baseline, e reportamos $p$-valores, intervalos de confiança
de \SI{95}{\percent} para a diferença média e tamanhos de efeito $d_z$.
A distribuição das diferenças por semente é exibida na Figura~\ref{fig:paired}.
...
Um episódio ilustrativo de melhor cenário (semente $340$) é renderizado em
perspectiva 3D apenas para contrastar espacialmente o estado final; a
inferência quantitativa permanece baseada nas $100$ sementes compartilhadas.
